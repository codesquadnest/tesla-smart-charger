...
class TeslaAPI:
    ...
    def __init__(self, charger_config: ChargerConfig) -> None:
        ...
        if "error" in cfg:
            # Avoid crashing on import or early construction; methods re-validate.
            tsc_logger.warning("TeslaAPI initialized before config was loaded; proceeding with defaults.")
            self.http_proxy = ""
        else:
    ...
    def get_vehicle_data(self) -> dict:
        ...
        if not vehicle_id:
            raise HTTPException(status_code=400, detail="Missing teslaVehicleId in config")
