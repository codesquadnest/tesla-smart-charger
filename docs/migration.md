...
If you are setting up for the first time, follow the
...
> **Note:** `region` defaults to `eu` and `voltage` to `230` V during migration.
> If you are outside the EU, review these in **Settings** afterwards.
...
If you need to go back to v1, check out your last v1 version — a v1 release tag
or the commit you upgraded from (see the repository's
[Releases](https://github.com/codesquadnest/tesla-smart-charger/releases) page):

```bash
git checkout <v1-tag-or-commit>
# Restore the v1 docker-compose.yaml volume mount (./config.json:/app/config.json)
docker compose up --build -d
```

Then restore your backed-up config:
